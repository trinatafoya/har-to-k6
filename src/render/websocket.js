...
function changeMessageTimes(messages) {
  ...
  if (messages.length != 0) {
    ...
    for (let i = 1; i < messages.length; i++) {
      messagesWithNewTimes[i].time = (messages[i].time - messages[i - 1].time) * 1000 // 1000 millisecond multiplier
    }
    var sum = messagesWithNewTimes.reduce((accum, curr) => {return accum + curr.time}, 0)
...
function ws_send_messages(factor) {
  ...
  let socket_function = [' function (socket) {','let timings = { } ', 'const messages = [']
  let send_messages = [
  ...
  if (messages) {
    for (const message of messages) {
      if (message.type === 'send') {
        ...
        if (parsed_data.type === "connection_init" || parsed_data.type === "start" || !factor.addSleep ) {
